...
```{r}
stripplot(gExp ~ group | gene, prepareData(head(rownames(CDhits), 3)), jitter.data = TRUE, auto.key = TRUE, type = c("p", "a"), grid = TRUE)
```
...
```{r}
stripplot(gExp ~ group | gene, prepareData(head(rownames(UChits), 3)), jitter.data = TRUE, auto.key = TRUE, type = c("p", "a"), grid = TRUE)
```
